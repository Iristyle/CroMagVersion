...
function Restore-Nuget
{
    ...
    Start-Process 'msbuild' -ArgumentList "NugetRestore.proj" -WorkingDirectory (Join-Path (Get-CurrentDirectory) '.nuget') -NoNewWindow -Wait
    Get-NugetPath
}
...
function Pack-And-Push
{
    ...
    Get-ChildItem -Path $currentDirectory -Filter *.nuspec -Recurse | 
        ...
        % {
            ...
            $cmdLine = if (Test-Path $csproj) { "pack $csproj -Prop Configuration=Release -Exclude '**\*.CodeAnalysisLog.xml'" }
            else { "pack $_"}
